      ...
      <div className="container-px mx-auto max-w-5xl">
        <Reveal className="mx-auto max-w-2xl text-center">
          ...
          <h2 className="mt-5 text-3xl font-extrabold tracking-tight sm:text-4xl lg:text-5xl">
            VEJA O <span className="text-gradient-red">EFEITOS PRO+</span> EM AÇÃO
          </h2>
        </Reveal>
        ...
        <Reveal delay={120} className="relative mx-auto mt-12 max-w-3xl">
          ...
          >
            ...
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(228,25,47,0.16),transparent_60%)]" />
            {[...Array(8)].map((_, i) => (
              <span
                key={i}
            ...
            <div className="relative flex flex-col items-center gap-4">
              <span className="flex h-20 w-20 items-center justify-center rounded-full border border-red-400/30 bg-red-500/15 backdrop-blur-md transition-all duration-300 group-hover:scale-105 group-hover:bg-red-500/25 group-hover:shadow-red">
                <Play size={28} className="ml-1 text-red-400" fill="currentColor" />
              </span>
          ...
          <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
            {CATEGORIES.map((c, i) => (
              <Reveal key={c.label} delay={i * 80}>
                <div className="group flex items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/[0.02] py-3.5 text-xs font-semibold text-ink-200 transition-all duration-300 hover:-translate-y-0.5 hover:border-red-500/25 hover:bg-white/[0.04]">
                  <c.icon size={14} className="text-red-400 transition-transform duration-300 group-hover:scale-110" />
                  {c.label}
                </div>
              </Reveal>
            ))}
